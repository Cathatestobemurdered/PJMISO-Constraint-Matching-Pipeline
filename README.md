...
The script uses `rapidfuzz` when available for faster fuzzy matching. A backup scoring function is included in the pipeline so the code can still run in a basic Python environment.

## Notes and Limitations

- Market is used as the anchor source.
- The matching output should be reviewed for low-confidence records.
- Some Pano records are historical, so the output includes `pano_latest` and `pano_stale`.
- Some Dayzer names do not include a contingency part, so those matches rely more heavily on the facility name.
- A future improvement would be two-sided reconciliation to check whether multiple Market rows map to the same Dayzer or Pano candidate.
